...
function openAddKiosk() {
  ...
  $('kioskId').value = '';
  ['kioskComputerName','kioskIp','kioskDescription','kioskManager','kioskManagerEmail','kioskNotes'].forEach(id => $(id).value = '');
  $('kioskAreaId').value = '';
  $('kioskActive').checked = true;
  $('kioskAlertOffline').checked = false;
...
function openEditKiosk(id) {
  ...
  $('kioskManagerEmail').value   = k.manager_email      || '';
  $('kioskNotes').value          = k.notes             || '';
  $('kioskActive').checked       = k.is_active === 1 || k.is_active === true;
  $('kioskAlertOffline').checked = k.alert_offline === 1 || k.alert_offline === true;
  openModal('kioskModal');
}
...
async function saveKiosk() {
  ...
  const data = {
    ...
    manager_email:    $('kioskManagerEmail').value.trim(),
    notes:            $('kioskNotes').value.trim(),
    is_active:        $('kioskActive').checked ? 1 : 0,
    alert_offline:    $('kioskAlertOffline').checked ? 1 : 0
  };
